...
from .utilis import get_client_ip
...
class ObjectViewed(models.Model):
    ...
    content_type    = models.ForeignKey(ContentType, null=True, on_delete=models.SET_NULL) # User, Product, Cart, Address
...
def post_save_user_changed_receiver(sender, instance, created, *args, **kwargs):
    if not created:
        if instance.is_active == False:
            qs = UserSession.objects.filter(user=instance, ended=False,  active=False).exclude(id=instance.id)
            for i in qs:
                i.end_session()
